...
class Deeplab_V3_Plus(nn.Module):
    ...
    def __init__(self, fusion,class_num=4, drop_rate=0):
        ...
        if self.fusion:
            self.proj_head = ProjectionHead(dim_in=256, proj_dim=256)
    def forward(self, x):
        ...
        out_feature=interpolate(y, scale_factor=4, mode='bilinear', align_corners=True)
        y = self.conv3(out_feature)
        y = self.conv4(y)
        y = self.conv5(y)
        y = softmax(y, dim=1)
        if self.fusion:
            emb=self.proj_head(out_feature)
            return y,emb
        else:
            return y
