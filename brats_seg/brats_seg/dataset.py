...
def data_loader(args, mode):
    ...
    if mode=='test':
        dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=shuffle, num_workers=nw)
        return dataloader
    else:
...
class TrainSet(torch.utils.data.Dataset):
    def __init__(self, args,prefix='../../data/sd0809/BraTS2020', transform=None):
        json_path = 'brats20.json'
        self.data_list = self.load_json(json_path)['train']
        self.prefix = prefix
        ...
        for data_item in self.data_list:
            ...
            label_path = data_item['label']
            for slice_idx in range(79, 129):  # 切片索引从79到128
                self.data_info.append((selected_images, label_path, slice_idx))
    ...
    def __getitem__(self, idx):
        image_paths, label_path, slice_idx = self.data_info[idx]

        # 加载图像和标签的切片数据
        
        images = [self.load_slice(os.path.join(self.prefix, image_path), slice_idx) for image_path in image_paths]
        label = self.load_slice(os.path.join(self.prefix, label_path), slice_idx)
        ...
        label = torch.tensor(label.astype(np.int32), dtype=torch.long).permute(1, 0)
        
        slice_images = F.interpolate(slice_images.unsqueeze(0), size=(256, 256), mode='bilinear', align_corners=False).squeeze(0)
    ...
    def load_slice(self, file_path, slice_idx):
        # 使用nibabel的内存映射功能加载NIfTI文件，并提取指定的切片
        nifti_image = nib.load(file_path, mmap=True)
        slice_data = np.asarray(nifti_image.dataobj[:, :, slice_idx])
        return slice_data
...
class TestSet(torch.utils.data.Dataset):
    def __init__(self, args,prefix='../../data/sd0809/BraTS2020', transform=None):
        json_path = 'brats20.json'
        self.data_list = self.load_json(json_path)['test']
        self.prefix = prefix
    ...
    def __getitem__(self, idx):
        ...
        return slice_images, label#,label_path,slice_idx
...
class ValidSet(torch.utils.data.Dataset):
    def __init__(self, args,prefix='../../data/sd0809/BraTS2020', transform=None):
        json_path = 'brats20.json'
        self.data_list = self.load_json(json_path)['valid']
        self.prefix = prefix
